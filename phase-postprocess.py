...
def phase_postprocess(plot_all=False):
    ...
    if plot_all:
        ...
        axs[0].yaxis.set_major_locator(plt.MultipleLocator(
            (max(motor_vel) - min(motor_vel))*1.1 / 4))
        axs[0].yaxis.set_minor_locator(plt.MultipleLocator(
            (max(motor_vel) - min(motor_vel))*1.1 / 12))
    ...
    if plot_all:
        ...
        axs[0].grid(b=True, which='minor', axis='y', linestyle=':')
        axs[0].yaxis.set_major_locator(plt.MultipleLocator(
            (max(motor_pos) - min(motor_pos))*1.2 / 4))
        axs[0].yaxis.set_minor_locator(plt.MultipleLocator(
            (max(motor_pos) - min(motor_pos))*1.2 / 12))
        axs[0].spines['top'].set_visible(False)
        axs[0].spines['right'].set_visible(False)
        axs[0].spines['left'].set_visible(False)
        ...
        axs[1].yaxis.set_major_locator(
            plt.MultipleLocator((max(diff) - min(diff))*1.2 / 4))
        axs[1].yaxis.set_minor_locator(
            plt.MultipleLocator((max(diff) - min(diff))*1.2 / 12))
        axs[1].spines['top'].set_visible(False)
        axs[1].spines['right'].set_visible(False)
        axs[1].spines['left'].set_visible(False)
        #axs.legend()
    ...
    if fit_angle <= 2*np.pi:
        ...
        l4, =axs.plot(fit_angle, p1(fit_angle), color='r', marker='x', linestyle='None')
    else:
        fit_angle = fit_angle -2*np.pi
        axs.set_xlim(-np.pi,np.pi)
        l4, =axs.plot(fit_angle, p2(fit_angle), color='r', marker='x', linestyle='None')
    ...
    axs.spines['right'].set_visible(False)
    axs.spines['left'].set_visible(False)
    axs.legend(handles=(l1,l2,l3,l4),labels=('Round 1','Round 2', 'Fitted curve','best ph_angle'))
    if plot_all:
        plt.show()
